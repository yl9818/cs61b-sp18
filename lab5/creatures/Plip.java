    ...
    public Action chooseAction(Map<Direction, Occupant> neighbors) {
        ...
        boolean anyClorus = false;
         ...
         for (Map.Entry<Direction, Occupant> neighbor: neighbors.entrySet()) {
             ...
             } else if (neighbors.get(neighbor.getKey()).name().equals("clorus")) {
                 anyClorus = true;
             }
         }
        ...
        if (this.energy >= 1.0) {
            Direction d = emptyNeighbors.get(StdRandom.uniform(emptyNeighbors.size()));
            return new Action(Action.ActionType.REPLICATE, d);
        }
        if (anyClorus) {
              boolean move = Math.random() < moveProbability;
              if (move) {
                  Direction d = emptyNeighbors.get(StdRandom.uniform(emptyNeighbors.size()));
